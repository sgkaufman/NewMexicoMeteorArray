...
# this verrsion includes one argument $1, specifiying the station ID

printf "\nFlushNMqueue.sh 25-Jul, 2022, byte count 872 : Flush NM upload queue if needed\n"
...
do
    if test $( find ""$HOME"/RMS_data/"$1"/NM_FILES_TO_UPLOAD.inf" -mmin +10 ); then
	old_enough=1
    else
...
if [ -s "$HOME"/RMS_data/"$1"/NM_FILES_TO_UPLOAD.inf ] ; then
   cd "$HOME"/source/RMS
   printf "Flushing queue of files to upload to NM Server\n"
   python -m RMS.FlushNMqueue_tmp "$1"
else
